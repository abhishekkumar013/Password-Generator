...
import BouncyCheckbox from 'react-native-bouncy-checkbox'; // for form alidation
...
const passwordSchema = Yup.object().shape({
  passwordLength: Yup.number()
    ...
    .max(16, 'ppassword length must be less than 16')
...
export default function App() {
  ...
  const generatePassword = (passwordLength: number) => {
    let str = '';
    str += lowerCase ? 'abcdefghijklmnopqrstvwxyz' : '';
    str += upperCase ? 'ABCDEFGHIJLMNOPQRSTUVWXYZ' : '';
    str += numbers ? '0123456789' : '';
    str += symbols ? '@#$%&*{}[]=-^><~?' : '';

    const generatedpassword = createPassword(str, passwordLength);
